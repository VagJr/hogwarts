...
// ==============================================================================
// INICIALIZAÇÃO DE GRUPOS E FUNÇÕES GLOBAIS DE RECOMPENSA
// ==============================================================================
    ...
    if (liderId) {
        ...
        membrosOnline.forEach(mId => {
            ...
            if(membro) {
                ...
                io.to(`priv_${mId}`).emit('nova_mensagem', { canal: 'zona', autor: 'SISTEMA', texto: `Grupo derrotou o alvo: +${xpDividido} XP e +${goldDividido} G` });
            }
        });
    } else {
        // Lógica Solo Normal
...
// ==============================================================================
// LÓGICA DO SERVIDOR E BASE DE DADOS
// ==============================================================================
...
app.post('/api/castelo/cozinha_stealth', (req, res) => {
    ...
    if(win) {
        a.fome = 100;
        a.energia = Math.min(100, a.energia + 20); 
        core._salvarUrgente();
...
app.post('/api/registrar', async (req, res) => { 
    try { 
        ...
        if (r.sucesso) { await core._salvarUrgente(); }
        res.json(r); 
    } catch(e) { res.status(500).json({erro: "Erro ao acessar o Grande Livro."}); } 
});
...
app.post('/api/livros/abrir', async (req, res) => {
    ...
    } catch(e) { res.status(500).json({erro: "A biblioteca trancou as portas."}); }
});
...
app.post('/api/biblioteca/gerar_livro', async (req, res) => {
    try {
        ...
        res.json(livro);
    } catch(e) { res.status(500).json({erro: "As estantes da biblioteca trancaram-se."}); }
});
...
app.post('/api/biblioteca/avaliar_tese', async (req, res) => {
    try {
        ...
        if (!r || !r.aprovado) {
            let msg = (r && r.feedback) ? r.feedback : "A Consciência não compreendeu a tua magia.";
        ...
        a.elos.sabedoria = (a.elos.sabedoria || 1000) + elo;
        ...
        if (r.aprovado && r.feitico) {
            ...
            const novoFeiticoSanitizado = {
                ...
                custoMana: Math.min(10, custoReal),
                poderBase: poderReal,
        ...
        res.json(r);
    } catch(e) { res.status(500).json({erro: "A sala da biblioteca trancou as portas."}); }
});
...
app.post('/api/biblioteca/publicar', (req, res) => {
    ...
    const a = core.alunos[id];
    core.livrosPublicos.push({ id: Date.now(), autor: a.nome, titulo, conteudo, feiticoOriginalId: feiticoId });
...
app.post('/api/quarto/estado', (req, res) => {
    const { id, movelId, equipado, x, y, rotacao } = req.body;
    const a = core.alunos[id];
    if (a && a.inventario.mobilia) {
        ...
        if (m) {
            ...
            if (rotacao !== undefined) m.rotacao = rotacao;
            core._salvarUrgente();
        }
    }
    res.json({ sucesso: true });
...
app.post('/api/estufa/regar', (req, res) => {
    ...
    res.json(core.regarEstufa(id, poteId));
});

app.post('/api/estufa/curar', (req, res) => {
    const { id, poteId } = req.body;
    if (!core.alunos[id]) return res.json({ erro: "Estudante não encontrado." });
    res.json(core.curarPragaEstufa(id, poteId));
});
...
app.post('/api/baile/recompensa', (req, res) => {
    ...
    const a = core.alunos[id];
    if(!a) return res.json({erro: "Erro"});
    let eloGained = Math.floor(score / 500); 
...
app.post('/api/aulas/folhear', async (req, res) => {
    try {
        ...
        res.json({ sucesso: true, titulo: livroDoc.nomeLivro, capitulos: livroDoc.capitulos, capituloSugerido: relogio.capituloAtual });
    } catch(e) { res.status(500).json({erro: "Erro ao abrir livro de aula."}); }
});

app.post('/api/aulas/iniciar_imersiva', async (req, res) => { try { const r = await core.assistirAula(req.body.id); if (!r.erro) forcarSyncAluno(req.body.id); res.json(r); } catch(e) { res.status(500).json({erro: "Erro na secretaria escolar."}); } });
...
app.post('/api/inventario/abrir_sapo', (req, res) => {
    try {
        ...
        a.focoAtual = Math.min(a.maxFoco, (parseInt(a.focoAtual) || 0) + 5); 
...
app.post('/api/dungeon/iniciar', async (req, res) => {
    ...
    } catch(e) { res.status(500).json({erro: "A masmorra colapsou."}); }
});
...
app.post('/api/dungeon/fugir_floresta_seguro', (req, res) => {
    const a = core.alunos[req.body.id];
    core.florestaEngine.extrairLootEVaz(a); 
...
io.on('connection', (socket) => {
    ...
    // SISTEMA DE GRUPOS E CONVITES (Que causava o erro ReferenceError)
    ...
    socket.on('mmo_interagir_objeto', async (dados) => {
        ...
        if (mobIdx !== -1) {
            ...
            io.to(`zona_${socket.zonaAtual}`).emit('nova_mensagem', {
                canal: 'zona', autor: '⚔️ COMBATE', texto: `${a.nome} lançou um feitiço no ${mob.nome}! (-${dano} HP)`
            });
        ...
        if (itemIdx !== -1) {
            ...
            io.to(`zona_${socket.zonaAtual}`).emit('mmo_item_coletado', { 
                id: dados.alvoId, texto: `🖐️ ${a.nome} foi mais rápido e pegou o [${item.nome}]!` 
            });
        }
        core._salvarUrgente();
    });
        ...
        if (dados.tipo === 'coleta') {
            ...
            if (itemIdx !== -1) {
                ...
                io.to(`zona_${dados.zona}`).emit('mmo_item_coletado', { 
                    id: dados.idAlvo, texto: `🖐️ [${novoItem.nome}] foi recolhido por ${a.nome}!` 
                });
    ...
    socket.on('mensagem_chat', async (dados) => {
            ...
            if (respIA && respIA.personagem !== "Nenhum" && respIA.texto) {
                setTimeout(() => { 
                    ...
                    if (respIA.spawnMob && sala) {
                        const a = core.alunos[dados.remetenteId]; // <-- Usar o Id da pessoa certa
                        const hpEscalado = 400 + ((a ? a.nivel : 1) * 50);
                        const mob = core._gerarMonstroRapido(hpEscalado, dados.zona, false);
                        mob.nome = respIA.spawnMob; 
        ...
        if (dados.canal === 'aula') {
            ...
            if(relogio.professorAtivo !== 'Nenhum') {
                ...
                const anoLetivo = a ? a.anoLetivo : 1; 
